...
%% EKF solution
  ...
  % Set up figure
  figure(1); clf
  H = @(x) [(1/(1 + ((x(2) - S1(2)) / (x(1) - S1(1)))^2)) * ((x(2) - S1(2))) * (-1/(x(1) - S1(1))^2),(1/(1 + ((x(2) - S1(2)) / (x(1) - S1(1)))^2)) * (1/(x(1) - S1(1))), 0, 0;
            (1/(1 + ((x(2) - S2(2)) / (x(1) - S2(1)))^2)) * ((x(2) - S2(2))) * (-1/(x(1) - S2(1))^2),(1/(1 + ((x(2) - S2(2)) / (x(1) - S2(1)))^2)) * (1/(x(1) - S2(1))),0, 0];
  ...
  for k=1:steps
   ...
   %Prediction
    ...
    vk = Theta(:,k) - [atan2((m2_pred(2,1)-S1(2,1)), m2_pred(1,1)-S1(1,1));atan2((m2_pred(2,1)-S2(2,1)), m2_pred(1,1)-S2(1,1))];
    Sk = H(m2_pred)*P2_pred* H(m2_pred)' + R;
    Kk = P2_pred*H(m2_pred)'*inv(Sk);
    m2 = m2_pred+ Kk*vk;
    P2 = P2_pred - (Kk* Sk* Kk');
