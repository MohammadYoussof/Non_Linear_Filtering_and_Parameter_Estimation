...
%% Kalman filter
  ...
  for k=1:steps
    ...
    %m2 = m2;
    [m2,P2] = kf_predict(m2,P2,A,Q);
    [m2,P2] = kf_update(m2,P2,Y(:,k),C,r);
...
%% Stationary Kalman filter solution
  ...
  %P_bar = A*P3*A'+Q;
  P_bar = A*P3*A'+Q- A*P_bar*C'*inv(C*P_bar*C'+r)*C*P_bar*A';
...
%    warning('You should add the Kalman filtering equations.')
  ...
  %%
figure;clf;
plot(T,EST1(1,:),'--',T,EST2(1,:),'-',T,EST3(1,:),'o');
    legend('Baseline','Kalman filter signal','Stationary kalman filter');
    xlabel('Time step'); ; title('\bf Estimations')